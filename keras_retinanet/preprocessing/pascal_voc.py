...
voc_classes = {
    'bicycle':1, 'bus':2, 'train':3, 'motorcycle':4, 'car':5, 
    'person':6, 'truck':7, 'autorickshaw':8, 'trailer':9, 'caravan':10, 'rider':11, 'rickshaw':12, 'dummy':0
}

skip_classes = ['vehicle fallback','traffic sign', 'traffic light', 'animal']
...
class PascalVocGenerator(Generator):
    ...
    def __parse_annotation(self, element):
        ...
        if class_name not in self.classes or class_name in self.skip_class:
            label = ' '
            box = np.zeros((4,))
            return box, label
    ...
    def __parse_annotations(self, xml_root):
        ...
        """
        annotations = {'labels': np.zeros((len(xml_root.findall('object')),)), 'bboxes': np.zeros((len(xml_root.findall('object')), 4))}
        ...
        annotations['bboxes'] = annotations['bboxes'][~np.all(annotations['bboxes'] == 0, axis=1)]
        annotations['labels'] = annotations['labels'][np.where(annotations['labels'] != 0)]
        return annotations
